...
def get_date_site_lists(site_file_path, data_file_path, fmt_date_list):
    ...
    for site in sites_df.index:
        ...
        if f_name in os.listdir(data_file_path):
            ...
            bool_df[name] = map(lambda x: x in site_fmt_date_list, fmt_date_list)
    d = {}
    bool_df = bool_df.T
...
fmt_date_list = get_access_names_from_dates(date_list)

# Make list of time_date file names already written to existing file
list_dict = get_date_site_lists(site_file_path, data_file_path, fmt_date_list)
