...
for row_num in range(start_row, sheet.nrows):
    ...
    for var in ['Latitude', 'Longitude', 'Altitude']:
        bom_sites_info[ozflux_site_name][var.lower()] = this_row[header_list.index(var)]
    for index in range(1, 5):
        stn_id_col = header_list.index('BoM_ID_{}'.format(str(index)))
        try:
            bom_id = str(int(this_row[stn_id_col]))
            bom_sites_info[ozflux_site_name][bom_id] = {}
            for var in ['Latitude', 'Longitude', 'Altitude', 
                        'Distance', 'Name']:
                id_col = header_list.index('{}_{}'.format(var, str(index)))
                try:
                    var = alias_dict[var]
                except KeyError:
                    pass
                bom_sites_info[ozflux_site_name][bom_id][var.lower()] = this_row[id_col]
        except:
            continue
